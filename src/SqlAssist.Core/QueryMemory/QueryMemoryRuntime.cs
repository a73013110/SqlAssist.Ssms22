...
public interface IQueryMemoryTimerFactory
{
...
public interface IQueryMemoryRuntimeLog
{
...
public sealed class QueryMemoryRuntimeOptions
{
    public static QueryMemoryRuntimeOptions Default { get; } = new();
...
/// </remarks>
public sealed class QueryMemoryRuntime
{
    private readonly Func<CancellationToken, Task<IQueryMemoryStorage>> _openStorage;
    private readonly Func<QueryMemoryLeaseOwner> _owner;
    private readonly QueryMemoryLeaseReaper _reaper;
    private readonly IQueryMemoryTimerFactory _timers;
    private readonly IQueryMemoryRuntimeLog _log;
    private readonly Func<DateTimeOffset> _clock;
    private readonly QueryMemoryRuntimeOptions _options;
    private readonly SemaphoreSlim _gate = new(1, 1);
    private readonly CancellationTokenSource _lifetime = new();
    private readonly object _sync = new();

    private QueryMemoryConfiguration _configuration = QueryMemoryConfiguration.Disabled;
    private QueryMemoryRuntimeStatus _status = QueryMemoryRuntimeStatus.Initial;
    ...
    /// <param name="openStorage">開啟一份儲存；取消代表宿主正在卸載。失敗以 <see cref="QueryMemoryStorageException"/> 分類為佳。</param>
    /// <param name="owner">本程序的租約擁有者；開啟儲存時才取得，探測失敗會走開啟失敗的路徑。</param>
    /// <param name="reaper">判斷過期租約的程序是否真的不在。</param>
    public QueryMemoryRuntime(Func<CancellationToken, Task<IQueryMemoryStorage>> openStorage, Func<QueryMemoryLeaseOwner> owner,
        QueryMemoryLeaseReaper reaper, IQueryMemoryTimerFactory timers, IQueryMemoryRuntimeLog log,
        Func<DateTimeOffset>? clock = null, QueryMemoryRuntimeOptions? options = null)
    {
        ...
        _options = options ?? QueryMemoryRuntimeOptions.Default;
    }

    /// <summary>狀態或世代改變時觸發，可能在任何執行緒；處理常式不得擲出，也不得同步等待本物件的操作。</summary>
    public event EventHandler<QueryMemoryRuntimeStatus>? StatusChanged;

    /// <summary>一筆擷取沒有保存時觸發，可能在 UI 或背景寫入器的執行緒上。</summary>
    public event EventHandler<QueryMemoryCaptureDroppedEventArgs>? CaptureDropped;

    public QueryMemoryRuntimeStatus Status => Volatile.Read(ref _status);
    ...
    /// <summary>套用新設定：開啟、只換政策或關閉。開啟失敗會擲出，狀態同時變成 <see cref="QueryMemoryRuntimePhase.OpenFailed"/>。</summary>
    public Task ApplyAsync(QueryMemoryConfiguration configuration)
    {
        ...
        if (!configuration.Enabled) Publish(status => status.With(QueryMemoryRuntimePhase.Disabled));
        else if (Volatile.Read(ref _state) is null) Publish(status => status.With(QueryMemoryRuntimePhase.Opening));
    ...
    public QueryMemoryEnqueueResult TryEnqueue(QueryMemoryCapture capture)
    {
        if (capture == null) throw new ArgumentNullException(nameof(capture));
        var state = Volatile.Read(ref _state);
        if (state is null) return QueryMemoryEnqueueResult.Stopped;

        var result = state.Writer.TryEnqueue(capture, state.Policy);
        if (result == QueryMemoryEnqueueResult.QueueFull) Drop(state, QueryMemoryCaptureDrop.QueueFull, "SQL Memory 拒絕擷取：" + result);
        else if (result == QueryMemoryEnqueueResult.SnapshotTooLarge) Drop(state, QueryMemoryCaptureDrop.SnapshotTooLarge, "SQL Memory 拒絕擷取：" + result);
        return result;
    }
    ...
    public Task<QueryMemoryPage<FavoriteQueryEntry>> ReadFavoriteQueriesAsync(FavoriteQueryRequest request, CancellationToken cancellationToken) =>
        UseAsync((storage, token) => storage.ReadFavoriteQueriesAsync(request, token), cancellationToken);

    public Task<IReadOnlyList<string>> ReadConnectionFacetsAsync(QueryConnectionFacetRequest request,
    ...
    public Task<FavoriteQueryWriteResult> WriteFavoriteQueryAsync(FavoriteQueryWrite write, CancellationToken cancellationToken) =>
        UseAsync((storage, token) => storage.WriteFavoriteQueryAsync(write, token), cancellationToken);
    ...
    public Task<FavoriteQueryWriteResult> EditFavoriteQuerySqlAsync(FavoriteQueryEdit edit, CancellationToken cancellationToken) =>
        UseAsync((storage, token) => storage.EditFavoriteQuerySqlAsync(edit, token), cancellationToken);
    ...
    public Task<FavoriteQueryWriteResult> DeleteFavoriteQueryAsync(Guid favoriteQueryId, Guid expectedVersion, CancellationToken cancellationToken) =>
        UseAsync((storage, token) => storage.DeleteFavoriteQueryAsync(favoriteQueryId, expectedVersion, token), cancellationToken);
    ...
    /// </remarks>
    public async Task<QueryMemoryUsage> CompactAsync(CancellationToken cancellationToken)
    {
    ...
    public async Task<bool> ShutdownAsync(TimeSpan timeout)
    {
        ...
        lock (_sync)
        {
            ...
            Volatile.Write(ref _configuration, QueryMemoryConfiguration.Disabled);
        }
        ...
        Publish(status => status.With(QueryMemoryRuntimePhase.Disabled));
        return closed;
    }
    ...
    // 長時間的全文搜尋才不會拖住卸載。
    private async Task<T> UseAsync<T>(Func<IQueryMemoryStorage, CancellationToken, Task<T>> operation,
        CancellationToken cancellationToken)
    {
        if (!Volatile.Read(ref _configuration).Enabled || Volatile.Read(ref _state) is not { } state)
            throw new QueryMemoryStorageException(QueryMemoryStorageErrorKind.Unavailable, Status.Message);
            ...
            !cancellationToken.IsCancellationRequested && state.Lifetime.IsCancellationRequested)
        {
            ...
            throw new QueryMemoryStorageException(QueryMemoryStorageErrorKind.Unavailable,
    ...
    private async Task<bool> OpenOrUpdateLockedAsync(QueryMemoryConfiguration configuration)
    {
        if (_state is { } existing)
        {
            // 儲存與維護排程都不重開；只換擷取政策與保留計畫，計畫指紋變了游標才跟著重來。
            existing.Reconfigure(configuration, _options);
            Publish(status => status.With(QueryMemoryRuntimePhase.Ready));
            return true;
        }

        Publish(status => status.With(QueryMemoryRuntimePhase.Opening));
        IQueryMemoryStorage storage;
        try
        {
            var owner = _owner();
        ...
        Publish(status => status.With(QueryMemoryRuntimePhase.Ready, nextGeneration: true));
    ...
    private async Task<bool> CloseLockedAsync(State? failed, Func<TimeSpan> remaining)
    {
        ...
        Publish(status => status.With(failed is null ? QueryMemoryRuntimePhase.Disabled : QueryMemoryRuntimePhase.WriterFailed,
            nextGeneration: true));
    ...
    private void OnBusy(State state, QueryMemoryCapture capture, QueryMemoryStorageException error) =>
        Drop(state, QueryMemoryCaptureDrop.StorageBusy,
            $"SQL Memory 儲存持續忙碌，放棄擷取 {capture.Kind}（SQLite {error.ErrorCode}/{error.ExtendedErrorCode}）：{error.Message}");

    private void Drop(State state, QueryMemoryCaptureDrop drop, string diagnostic)
    {
        _log.Important(diagnostic);
        if (ReferenceEquals(Volatile.Read(ref _state), state))
            Publish(status => status.Phase == QueryMemoryRuntimePhase.Ready ? status.WithDrop(drop) : status);

        var first = drop switch
        {
            QueryMemoryCaptureDrop.StorageBusy => Interlocked.Exchange(ref state.BusyReported, 1) == 0,
            QueryMemoryCaptureDrop.QueueFull => Interlocked.Exchange(ref state.QueueFullReported, 1) == 0,
            _ => true,
        };
        CaptureDropped?.Invoke(this, new QueryMemoryCaptureDroppedEventArgs(drop, first));
    }

    private void Publish(Func<QueryMemoryRuntimeStatus, QueryMemoryRuntimeStatus> change)
    {
        QueryMemoryRuntimeStatus current, next;
        do
        {
    ...
    private sealed class State
    {
        ...
        private QueryMemoryPolicy _policy;

        private State(IQueryMemoryStorage storage, CancellationTokenSource lifetime, QueryMemoryBackgroundWriter writer,
        ...
        public IQueryMemoryStorage Storage { get; }
        ...
        public QueryMemoryPolicy Policy => Volatile.Read(ref _policy);

        public static State Create(IQueryMemoryStorage storage, QueryMemoryConfiguration configuration, QueryMemoryLeaseOwner owner,
            QueryMemoryLeaseReaper reaper, QueryMemoryRuntimeOptions options, DateTimeOffset now,
            Action<State, QueryMemoryCapture, QueryMemoryStorageException> busy)
        {
            var heartbeat = new QueryMemoryLeaseHeartbeat(storage, owner, options.HeartbeatInterval);
            var schedule = new QueryMemoryMaintenanceSchedule(now, options.StartupDelay, configuration.MaintenanceInterval,
                options.PendingDelay, configuration.IdleMinimumGap);
            var runner = new QueryMemoryMaintenanceRunner(storage, storage, reaper, owner, schedule, configuration.Plan,
                options.LeaseExpiry);

            State? created = null;
            // 租約明確隨每次提交傳入，不藏在儲存層；心跳重開後的下一筆就標上新租約。
            var writer = new QueryMemoryBackgroundWriter(
                new QueryMemoryProcessor(storage, new QueryRevisionEngine(), leaseId: () => heartbeat.LeaseId),
        ...
        /// </remarks>
        public void Reconfigure(QueryMemoryConfiguration configuration, QueryMemoryRuntimeOptions options)
        {

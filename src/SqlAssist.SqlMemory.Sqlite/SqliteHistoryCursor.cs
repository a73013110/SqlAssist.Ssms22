...
internal sealed class SqliteHistoryCursor
{
    ...
    public static string Encode(SqlHistoryRequest request, string storeId, long ticks, string key) =>
    ...
    public static SqliteHistoryCursor? Decode(SqlHistoryRequest request, string storeId)
    {
    ...
    private static SqlMemoryStorageException InvalidCursor(string message) =>
        new(SqlMemoryStorageErrorKind.InvalidCursor, message);

    private static string Fingerprint(SqlHistoryRequest request)
    {
        string Field(string? value) => SqliteFilterKey.Field(value);
        return SqlContent.Create(((int)request.Kind).ToString(CultureInfo.InvariantCulture) + ";" +

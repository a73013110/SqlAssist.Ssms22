...
internal static class SqliteContentRows
{
 ...
 AND NOT EXISTS(SELECT 1 FROM History WHERE ContentId=$id)";
    ...
    // QueryMemoryStorageSelfTest／ReadContent 的完整讀取驗證；取捨見 docs/sql-memory-storage.md。
    public static void WriteContent(SqliteConnection connection, SqliteTransaction transaction, QueryContent content)
    {
...
ON CONFLICT(ContentId) DO NOTHING;", ("$id", content.ContentId), ("$hash", content.ContentHash), ("$sql", bytes),
    ...
    public static string? WriteContext(SqliteConnection connection, SqliteTransaction transaction, QueryConnectionContext? context)
    {
        if (context == null) return null;
        var key = QueryContent.Create(SqliteFilterKey.Field(context.Server) + SqliteFilterKey.Field(context.Database)).ContentHash;
    ...
    public static QueryConnectionContext? ReadContext(SqliteDataReader reader, int start) => reader.IsDBNull(start) ? null :

...
internal static class SqliteMaintenanceStages
{
    ...
    public static SqliteMaintenanceStage[] For(QueryMemoryMaintenanceScan scan) =>
        scan == QueryMemoryMaintenanceScan.Full ? Full : Indexed;
 ...
 ORDER BY CreatedAt,EntryKey LIMIT $limit;";
    ...
    public const string FavoriteRevisionGroup = @"SELECT FavoriteQueryId FROM Revisions INDEXED BY IX_Revisions_Favorite
 WHERE FavoriteQueryId IS NOT NULL AND FavoriteQueryId>$group ORDER BY FavoriteQueryId LIMIT 1;";

    public const string FavoriteRevisions = @"SELECT RevisionId,CreatedAt FROM Revisions INDEXED BY IX_Revisions_Favorite
 WHERE FavoriteQueryId IS NOT NULL AND FavoriteQueryId=$group AND CreatedAt<$cutoff AND (CreatedAt,RevisionId)>($time,$key)
 ORDER BY CreatedAt,RevisionId LIMIT $limit;";

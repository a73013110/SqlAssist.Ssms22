...
/// <summary>
/// 卡片清單的頁尾：細線夾著的摘要膠囊、淡色說明，以及膠囊形的續頁按鈕（SQL Memory 與 SQL Search 共用）。
/// </summary>
/// <remarks>
/// 狀態與文案全部來自 <see cref="SqlListFooter"/>，這裡只負責呈現與動畫。
...
/// </remarks>
internal sealed class SqlListPager : StackPanel
{
    ...
    public SqlListPager()
    {
        ...
        heading.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star), MinWidth = 12 });
        heading.Children.Add(SqlAssistChrome.CreateListPagerRule());
        ...
        var right = SqlAssistChrome.CreateListPagerRule();
        ...
        Button = SqlAssistChrome.CreateListPagerButton();
        ...
        Button.Click += (_, _) => { if (_kind is SqlListFooterKind.More or SqlListFooterKind.ContinueSearch) LoadMoreRequested?.Invoke(this, EventArgs.Empty); };
    ...
    public SqlListFooterKind Kind => _kind;

    public string Summary => _summary.Text;

    public void Update(SqlListFooter footer)
    {
        ...
        Button.ToolTip = footer.Kind == SqlListFooterKind.ContinueSearch ? footer.Hint : null;
    ...
    {
        if (_kind == SqlListFooterKind.Loading || !Button.IsEnabled) offset = 0;
    ...
    private void UpdateSpin()
    {
        ...
        _spin.BeginAnimation(RotateTransform.AngleProperty,
            _kind == SqlListFooterKind.Loading && IsVisible && SqlAssistChrome.MotionEnabled

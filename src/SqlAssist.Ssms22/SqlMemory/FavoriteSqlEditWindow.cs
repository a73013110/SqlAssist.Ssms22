...
internal sealed class FavoriteSqlEditWindow : DialogWindow
{
    ...
    private readonly SqlFavoriteItem _entry;
    ...
    public FavoriteSqlEditWindow(SqlAssistPackage package, SqlFavoriteItem entry, string sql)
    {
        _package = package; _entry = entry; _editor = new SqlTextEditor(sql);
        SqlMemoryActions.ConfigureWindow(this, package, "編輯 SQL — " + entry.Favorite.Name, 900, 620);
        ...
        _apply.Click += (_, _) => _ = SqlMemoryActions.RunAsync(ApplyChangesAsync, Report);
        Closing += OnClosing;
    }
    ...
    private async Task ApplyChangesAsync()
    {
        ...
        try
        {
            var edit = new SqlFavoriteSqlEdit(_entry.Favorite.FavoriteId, _entry.Version, Guid.NewGuid(), _editor.Text, DateTimeOffset.UtcNow);
            var result = await SqlMemoryHost.Runtime.EditFavoriteSqlAsync(edit, _package.DisposalToken);

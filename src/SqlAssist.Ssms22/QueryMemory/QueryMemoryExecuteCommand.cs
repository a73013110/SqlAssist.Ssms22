...
/// </remarks>
internal static class QueryMemoryExecuteCommand
{
    ...
    public static void EnsureResolved(IServiceProvider? serviceProvider)
    {
        ...
        var resolved = SqlAssistPlatformGuard.Probe("取得執行查詢的命令識別碼", () =>
        {
            ...
            {
                SqlAssistDiagnostics.WriteAlways("取不到殼層命令名稱對應表，查詢記憶不擷取執行事件");
                return false;
            }
            ...
            if (ErrorHandler.Failed(mapping.MapNameToGUIDID(CanonicalName, out group, out commandId)))
            {
                SqlAssistDiagnostics.WriteAlways(
                    $"殼層不認得命令 {CanonicalName}，查詢記憶不擷取執行事件");
                return false;
            }

            _group = group;
            _commandId = commandId;
            SqlAssistDiagnostics.WriteAlways($"查詢記憶已接上執行命令 {CanonicalName}：{group:B}/{commandId}");
            return true;
        }, fallback: false);

...
internal sealed class FavoriteMetadataWindow : DialogWindow
{
    ...
    private readonly FavoriteQueryEntry? _existing;
    ...
    public FavoriteMetadataWindow(SqlAssistPackage package, QueryMemoryRow row, bool existing)
    {
        _package = package; _row = row; _existing = existing ? row.Favorite : null;
        QueryMemoryActions.ConfigureWindow(this, package, existing ? "編輯收藏資料" : "Add to Favorites", 580, 650);
        ...
        _scope.SelectedIndex = (int)(_existing?.Query.Scope ?? FavoriteQueryScope.Global);
        ...
        _submit.Click += (_, _) => _ = QueryMemoryActions.RunAsync(SubmitAsync, Report);
    ...
    private async Task SubmitAsync()
    {
        ...
        var connection = scope == FavoriteQueryScope.Global ? null
            : new QueryConnectionContext(_server.Text, scope == FavoriteQueryScope.Database ? _database.Text : "");
        var query = new FavoriteQuery(_existing?.Query.FavoriteQueryId ?? Guid.NewGuid(), _name.Text, _description.Text,
            revision, scope, connection);
        var write = new FavoriteQueryWrite(query, _existing?.Version);

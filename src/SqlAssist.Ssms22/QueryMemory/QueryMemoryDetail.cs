...
internal sealed class QueryMemoryDetail : UserControl, IDisposable
{
    ...
    public QueryMemoryDetail(SqlAssistPackage package, Action changed)
    {
        ...
        var open = Button("Open", "開啟至新 Query（不執行 SQL）", () => QueryMemoryActions.OpenQuery(_package, _viewer.Sql));
    ...
    public void Select(QueryMemoryRow? row, bool previewEnabled = true)
    {
        if (_disposed) return;

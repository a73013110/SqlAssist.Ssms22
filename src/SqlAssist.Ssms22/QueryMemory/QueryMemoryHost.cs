...
/// </summary>
/// <remarks>
/// 開啟、關閉、世代、故障恢復、心跳與維護排程都在 <see cref="QueryMemoryRuntime"/>，
...
internal static class QueryMemoryHost
{
    ...
    /// </summary>
    public static QueryMemoryRuntime Runtime { get; } = new(
        OpenStorageAsync,
        QueryMemoryProcessProbe.CurrentOwner,
    ...
    {
        var configuration = QueryMemoryConfiguration.From(SqlAssistSettingsStore.Current);
    ...
    private static void OnCaptureDropped(object? sender, QueryMemoryCaptureDroppedEventArgs drop)
    {
        if (drop.ShouldNotify && Volatile.Read(ref _package) is { } package)
            SqlAssistStatusBar.Show(package, drop.NotificationText);
    }

    private static async Task<IQueryMemoryStorage> OpenStorageAsync(CancellationToken cancellationToken) =>
        await IsolatedQueryMemoryRepository
    ...
    private sealed class GuardedTimers : IQueryMemoryTimerFactory
    {
    ...
    private sealed class DiagnosticsLog : IQueryMemoryRuntimeLog
    {

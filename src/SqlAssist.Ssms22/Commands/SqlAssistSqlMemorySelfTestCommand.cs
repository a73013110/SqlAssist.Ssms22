...
/// <summary>
/// 診斷命令「SQL Memory 儲存自我測試…」。
/// </summary>
/// <remarks>
/// 結果是一份要讀的報告（驗證了哪些項目、報告檔在哪裡），所以成敗都留在原生訊息框；
/// 「還在跑」則交給通知卡片，使用者可以回去編輯 SQL，不必盯著狀態列。
/// </remarks>
internal static class SqlAssistSqlMemorySelfTestCommand
{
    ...
    private static async Task ExecuteAsync(SqlAssistPackage package)
    {
        ...
        using (var notification = NotificationCenter.Default.Begin(NotificationCatalog.TestingSqlMemoryStorage,

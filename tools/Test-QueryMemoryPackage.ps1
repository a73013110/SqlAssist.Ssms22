...
$work = Join-Path $root ('artifacts/query-memory-package/' + [Guid]::NewGuid().ToString('N'))
$null = New-Item -ItemType Directory -Path $work
[IO.Compression.ZipFile]::ExtractToDirectory((Resolve-Path -LiteralPath $VsixPath).Path, $work)
$probe = Join-Path $work 'SqlAssist.QueryMemory.Probe.exe'
...
function Wait-Probe($Process, [string]$Name) {
    if (-not $Process.WaitForExit(60000)) {
        $Process.Kill()
        throw "Query Memory probe 逾時：$Name"
    }
    $Process.WaitForExit()
    ...
    if ($Process.ExitCode -ne 0) {
        ...
        throw "Query Memory probe 失敗：$Name，結束碼 $($Process.ExitCode)"
    }
}
...
Write-Host "Query Memory 封裝隔離驗證通過。紀錄：$work"

...
提交一筆建議會發生兩件事：先寫進一串名稱，某幾個位置再把那串名稱換成一整句。
前半段（補不補結構描述、加不加方括號、點號歸誰打）在下一節，後半段（展開的規則、
排版、值先填什麼，以及哪些欄位不能插進去）從[展開成整句](#展開成整句)開始。
...
描述」是中繼資料認出來的，[只認一次](completion-context.md#右對齊猜錯時整條往左挪)；

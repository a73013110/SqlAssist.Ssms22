...
代價是「開著」必須在那一顆自己身上看得出來，所以走 `CreateInputToggleStyle`：強調底加
...
filter、面板第一列的預設與續頁／排序、輸入列（輸入框吃剩餘寬度、收起的操作連間距一起讓開、
同一條視覺中心線）、框裡那種開關開著時不是與搜尋框同底的一圈外框、兩級分隔線（群距大於群內距、列首／換行／整群收起時孤線

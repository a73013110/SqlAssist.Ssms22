...
- Memory：`檔名 → 草稿／執行／收藏 → 次數（可選）→ 伺服器 → 資料庫 → 時間 → 操作`。
- Search：`物件名稱 → 物件類型 → 命中部位 → 伺服器 → 資料庫 → 操作`；類型是看得見的
...
- 操作放最右，與名稱、膠囊同一列，不自己占一列——自己一列等於每張卡片固定多一列高度。
  動作區用 `Visibility.Hidden` 預留寬度，hover 不跳版面。
- 靠右那幾組先宣告。`DockPanel` 依宣告順序量測，名稱先量的話，一個長名稱會把連線、時間與
  操作整組擠出這一列；讓得起的是可以 ellipsis 的名稱，不是固定寬的那幾顆。
...
filter、選取與捲動保存、四種狀態表面、Light／Dark／Blue 與 100／150／200% DPI。展開態的
Preview 開關屬於右欄、300 DIP 下每一組都還在列內（含操作）、窄版降級與 overflow 也要測。

...
本頁包含多段式名稱怎麼拆成一條路徑、以及右對齊猜錯時整條往左挪的規則。
提交時要補幾段見[插入文字](completion-insertion.md)。
...
**尾端的點號屬於正在輸入的那個名稱**，位置分析因此由整個名稱之前的東西決定位置。
當成一個算完的運算元的話，限定字之後一律是 `Any`，而那讓 `FROM a, dbo.` 在位置上
與 `SELECT dbo.` 沒有差別。數值不受影響：詞法分析把 `1.` 掃成一個數值詞元。

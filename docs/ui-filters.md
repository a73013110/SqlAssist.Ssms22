...
- Filter Flyout（`SqlFilterFlyout`）是**唯一一份**過濾面板：Search 的伺服器／資料庫／種類與
  ...
  Search 的未選是「連線預設」，Memory 的未選是「全部」，控制項不替它們挑一句。整批命令是
  **兩顆並排、一直亮著**的全選與全不選（`SetBulkCommands`），要不要**由宿主決定**，而且接線時
  講一次就好——字不跟狀態跑。不收起其中一顆：收起的那一版讓剩下那一顆滑進它的位置，同一個像素
  ...
  第三個入口，而它值得在命令列上再出現一次——命令列在捲動區外面，第一列那個預設卻是虛擬化清單的
  第 0 列，名稱上百個時捲下去就看不到了。
  **全選只作用在目前列出來的那一份**：面板把搜尋框的過濾字交給宿主（`SelectAllRequested`），
  ...
  的那幾頁。自己在宿主重寫一份比對的症狀是面板上看到五個、勾起來卻是七個。全不選相反，清的是
  整個維度而不是篩出來的那一份：它與第一列那個預設是同一件事。

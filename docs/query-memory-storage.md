...
- 現行 schema v3；v1 → v2 見 [Saved](query-memory-saved.md)，v2 → v3 計量與索引見[有界維護](query-memory-maintenance.md)。
...
已提供[有界期限清理與容量量測](query-memory-maintenance.md)，未接排程或編輯器事件。
筆數配額、Saved SQL 編輯流程、設定及 History UI 仍未完成。

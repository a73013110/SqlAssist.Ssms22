...
本頁供一般使用者安裝、驗證與初次操作；修改原始碼請看[建置與測試](development.md)。
...
安裝發布的 VSIX 不需要 Visual Studio 或 .NET SDK；其他 SSMS 版本、編輯器與作業系統
不在支援範圍。

## 你的資料去了哪裡

建議在本機計算；結構資訊只向目前查詢視窗連線的 SQL Server 查詢，沿用相同權限。
內容不送往雲端，也沒有 AI 模型參與。設定與診斷留在
`%LOCALAPPDATA%\SqlAssist.Ssms22`。
...
| 找某個名稱或字串在哪些物件裡 | **SqlAssist 工具列 → SQL Search**，或 **工具 → SqlAssist → SQL Search** |

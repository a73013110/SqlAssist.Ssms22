...
- `Title` 是動詞開頭的現在進行式短語，**不帶參數**，不含物件名稱、狀態與標點，
  不超過 14 個中文字。熱路徑不得組字串：通知被隱藏時那一份也已經配置完畢。
- 標題寫在 `NotificationCatalog.*.resjson`，每個 `X` 配一個過去式 `XDone`：英文的過去式不是加前綴。
  入列時以字串反查回目錄那一句（`NotificationTitle`），畫面、合併與統計都用它，換語言後已顯示的
  通知跟著重畫；提醒的標題、訊息與按鈕標籤也每次取值。呼叫端交來的 `Message` 仍是當下的字串。
...
- 提醒按鈕的識別字在 `NotificationActionIds`，其餘措辭是目錄的屬性或方法。
...
- **Navigation**：移至定義開始時還不知道游標下是哪個物件，物件名稱由後續的定義指令碼或新查詢視窗
  帶上。SQL Search 的移至定義與在物件總管中選取由 `SqlSearchActivation` 自己送，失敗原因寫在
  `Message`，還沒開始就被擋下的以同一個標題 `Post` 成失敗；成功而未連線時附上該連哪一台。
  SQL Search 的結果不在查詢視窗那一台時換成「開啟未連線的定義視窗」，`Source` 放來源伺服器：
...
- **SQL Memory**：逐處見[用量](sql-memory-usage.md)與 [UI](sql-memory-ui.md)；背景維護批次失敗沿用
  「維護」標題。列操作與多選動作的結果一律走通知（`SqlMemoryActions.Notify`）；等儲存的刪除用 `Begin`
  追蹤，主體是那一筆的名稱，多選時訊息寫筆數與衝突（`SqlMemoryDeleteReport.Summary`）。丟棄擷取走 `Post`，標題

...
每筆另帶 `SqlKeywordPosition`，語句級 DDL 不會混進 SELECT 欄位位置。排名與無前綴可見度
都在 `Core/Completion/SuggestionList`（分數層級在 `SuggestionScore`），測試守的就是產品走的那一條。
...
可能是名稱的位置（`FROM dbo.T |` 的別名）是軟選：不預先選中，只有 `Tab` 提交，`Enter` 照常
換行，第一次 `↓` 轉成硬選。哪些位置算見[子句邊界](completion-boundaries.md#名稱位置三分類)。
...
它只把平台項目轉接給 `SuggestionList`，並把命中區段交給平台畫粗體。
...
套用的是按著的與有命中的交集，在 `SuggestionList.Update`）：

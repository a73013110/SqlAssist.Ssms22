...
回到 [索引](index.md)。
欄位剖析與完整內容視窗的版面標準見[自製 UI 準則](ui-guidelines.md)。
...
| 欄位剖析 | 每一欄的 `NULL`／空字串／相異值數與範圍 | 一個可以複製成表格的視窗 |
...
前三個複製命令只放純文字、不放 HTML：它們是要貼進 SQL、工單或設定檔的原文，
多一份 HTML 會讓 Word 或郵件把 Markdown 貼成表格。

欄位剖析在寬表上的價值遠高於窄表。真正想知道的往往只是「哪幾欄整欄是 `NULL`、
哪幾欄從頭到尾只有一個值」——那兩件事看資料看不出來，看摘要一眼就有。統計的對象
刻意就是眼前這一份結果，不是資料表的全貌；後者下一句 `GROUP BY` 比較快也比較準。

欄位剖析可依欄名／型別篩選，或快速只看「全為 NULL」「僅單一值」；後者至少需兩列，
NULL 也算一種值。欄名在橫向捲動時固定，數量可按表頭排序；「複製目前表格」依目前
篩選與排序同時放 TSV 與 HTML 表格，貼進 Excel 就是一張表。
相異值數**包含 NULL**，不同於 SQL `COUNT(DISTINCT)`；
最小／最大值與字元範圍的排序依顯示文字，不是跨型別的 SQL 比較。
...
完整內容的「自動換行」只改變閱讀方式，不格式化 JSON／XML、不改原始空白或複製內容。
NULL 與空內容各有提示，提示本身不會被複製；也可選取一段後按 Ctrl+C。

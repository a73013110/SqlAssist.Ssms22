...
工具列的「用量」與 SqlAssist 選單的「SQL Memory 用量」開同一頁，取代主從區而不另開視窗。
...
| 開啟資料夾／保留設定 | 沿用復原卡片與設定入口 |

每批 200 個候選、一個 IMMEDIATE 交易，擷取與讀取插在批次之間；有刪除才截斷 WAL。
操作走 `SqlMemoryOperationGate`：期間動作全停用、頁首顯示不確定進度，Esc 不離開。
結果寫在工具窗狀態列並提醒壓縮後才縮檔，完成後重讀快照。
...
- 對話框改條件後 250 ms 重算，按鈕寫出筆數；取消是預設與初始焦點，清除為危險色幽靈按鈕並再確認一次。
...
- `SqlMemoryCapacityMonitor` 觀測維護批次、用量頁與手動整理回報的容量；分級改變觸發 `CapacityChanged`，
  工具列警示點在 Warning／Critical 顯示，出現時單次縮放 240 ms。

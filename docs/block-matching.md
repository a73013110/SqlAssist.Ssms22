...
| F 跨頁提示 | 開 | 視窗相對 Adornment，有界摘要，按鈕可返回起始行 |
...
資源。D／E 與編輯器底色至少 3:1；A 在保留指定字色與文字可讀性後盡量維持背景 3:1。
B 最高約 12% 不透明度並保護一般文字對比；不保證
...
目前是編輯器上方的一個 WPF 導覽按鈕，不是另一份 SQL 編輯器或原生黏附捲動。
游標所屬的最近區塊起始行已捲到上方時，顯示種類、行號與最多 160 字元摘要。
點擊可展開目的地所在的摺疊並返回起始行；不改 SQL，也不執行查詢。
目前不顯示完整祖先鏈、不預覽整段 SQL、不提供就地編輯，也不猜測 IF／WHILE 的語法歸屬。

沿用既有索引可擴充有限層數的父區塊導航、跳到結尾、區塊行數；這些尚未實作。
多行語法著色預覽或真正的多層黏附標題需要額外的排版、分類及遮擋處理，不能視為現有能力。

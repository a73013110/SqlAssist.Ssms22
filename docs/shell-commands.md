...
本頁包含 F12 怎麼接到殼層命令、濾鏡的回報要求、命令表幾種看起來正確卻安靜失效的寫法，
以及改完命令表為什麼一定要重新安裝。
平台硬規則見[平台護欄](rules-platform.md)。
...
而換成 moniker 圖示卻不加旗標又回到原值。反編譯 `Menus.cto` 也看得到 `SqlAssistMenu`
只剩 `Pict`／`IconAndText` 旗標，沒有 `<Icon>` 元素，同一份檔案的 `<Button>` 則保留得好好的。
所以工具選單上的 SqlAssist 是純文字，與 VS 內建子選單一致；品牌識別在工具列、右鍵選單
與關於視窗上。`tools/Test-CommandTable.ps1` 擋得下前者，後者只有這一段記錄得下來。

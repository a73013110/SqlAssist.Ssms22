...
/// 內容真的變動時仍要照常寫入。對照 SqlAssist.QueryMemory.Sqlite.Tests 專案下量到 commit
/// 耗時與資料列數的對應測試。
/// </summary>
public sealed class QueryRevisionEngineCapturePerformanceTests
{
    ...
    public void RepeatedUnchangedIdleCapturesStopProducingWritesAndStayCheap()
    {
        ...
        var first = engine.Prepare(Capture(text: text), null, Policy) ?? throw new InvalidOperationException();
        Assert.NotEmpty(first.Contents);
        Assert.NotNull(first.Recovery);

        var state = first.State;
        // 先跑一次讓 Recovery 已經指向目前內容；之後才是「內容不變」的穩態。
        var warm = engine.Prepare(Capture(2, text, seconds: 5), state, Policy) ?? throw new InvalidOperationException();
        ...
        {
            var write = engine.Prepare(Capture(sequence, text, seconds: (int)sequence * 5), state, Policy)
    ...
    public void RepeatedSmallEditsKeepProducingRecoveryWrites()
    {
        ...
        for (var i = 1; i <= 5; i++)
        {
            ...
            var write = engine.Prepare(Capture(i, text, seconds: i * 5), state, Policy)

...
public sealed class QueryMemoryRetentionPlanTests
{
    private static readonly DateTimeOffset Now = new(2026, 9, 12, 0, 0, 0, TimeSpan.Zero);

    private static QueryMemoryRetentionPlan Plan(int draftDays = 30, int executionDays = 180,
    ...
    {
        var daily = Plan().BuildLadder(Now, reclaimUnsavedDrafts: true)[0];
    ...
    public void TighteningStopsAtOneDayAndOneItem()
    {
        var ladder = Plan(draftDays: 1, executionDays: 1, unsavedDays: 1, quota: 1)
            .BuildLadder(Now, reclaimUnsavedDrafts: true);
    ...
    [Fact]
    public void TheUnsavedDraftDeadlineNeedsTheHostToBeRenewingTheHeartbeat()
    {
        var ladder = Plan().BuildLadder(Now, reclaimUnsavedDrafts: false);
    ...
    [InlineData(QueryMemoryStorageLimit.Gigabytes1, 1073741824L)]
    public void EachStorageLevelMapsToABoundedNumberOfBytes(QueryMemoryStorageLimit limit, long expected) =>
        Assert.Equal(expected, QueryMemoryRetentionPlan.ToContentBytes(limit));

    [Fact]
    public void UnlimitedStorageMeansNoCapacityTriggerAtAll() =>
        Assert.Null(QueryMemoryRetentionPlan.ToContentBytes(QueryMemoryStorageLimit.Unlimited));

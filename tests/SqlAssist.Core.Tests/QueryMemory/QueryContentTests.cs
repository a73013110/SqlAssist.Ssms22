...
public sealed class QueryContentTests
{
    [Theory]
    [InlineData("")]
    ...
    public void HashMatchesUtf16LittleEndian(string sql)
    {
        ...
        Assert.Equal(content.ContentId, QueryContent.Create(sql).ContentId);
    }
    ...
    public void LargeContentHashesAcrossBufferBoundary()
    {
        ...
        Assert.Equal(Convert.ToHexString(SHA256.HashData(Encoding.Unicode.GetBytes(sql))).ToLowerInvariant(), QueryContent.Create(sql).ContentHash);
    }
    ...
    public void DoesNotNormalizeDifferentSql(string first, string second) =>
        Assert.NotEqual(QueryContent.Create(first).ContentId, QueryContent.Create(second).ContentId);

    [Fact]
    public void UnpairedSurrogateDoesNotHashAsReplacementCharacter() =>
        Assert.NotEqual(QueryContent.Create(new string((char)0xd800, 1)).ContentId, QueryContent.Create("�").ContentId);
}

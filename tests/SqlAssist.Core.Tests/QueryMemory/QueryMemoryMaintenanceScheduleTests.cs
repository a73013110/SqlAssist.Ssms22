...
public sealed class QueryMemoryMaintenanceScheduleTests
{
    private static readonly DateTimeOffset Start = new(2026, 9, 12, 0, 0, 0, TimeSpan.Zero);

    private static QueryMemoryMaintenanceSchedule Schedule() => new(Start, TimeSpan.FromMinutes(2),
    ...
    {
        var schedule = new QueryMemoryMaintenanceSchedule(Start, TimeSpan.Zero, TimeSpan.FromMinutes(30),
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new QueryMemoryMaintenanceSchedule(Start,

...
public sealed class StorageSelfTestTests
{
    ...
    public async Task SelfTestProducesReportAndCanRunAgainWithoutLeakingDatabaseHandle()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            await QueryMemoryStorageSelfTest.RunAsync(directory, null, TestContext.Current.CancellationToken);
            ...
            Assert.Contains("Favorite Query CRUD、scope 分頁、搜尋、版本衝突與刪除後歷史保留", report);
            Assert.Contains("有界維護續跑、筆數配額、容量量測與無法回收時保護 Session head／Recovery", report);
            Assert.Contains("Session 心跳租約在租約還在時保護未存檔草稿", report);
            Assert.Contains("回收失效租約後才清除未存檔草稿，WAL 截斷與整理保留其餘內容", report);
    ...
    public async Task ExistingReportIsNeverOverwritten()
    {
        ...
        var report = Path.Combine(store.DirectoryPath, QueryMemoryStorageSelfTest.ReportFileName);
        File.WriteAllText(report, "保留診斷證據");
        await Assert.ThrowsAsync<IOException>(() => QueryMemoryStorageSelfTest.RunAsync(store.DirectoryPath, null, TestContext.Current.CancellationToken));
    ...
    public async Task ExistingDatabaseFailsWithReportInsteadOfReplacingData()
    {
        ...
        Assert.Contains("FAIL |", File.ReadAllText(Path.Combine(store.DirectoryPath, QueryMemoryStorageSelfTest.ReportFileName)));
    }
    ...
    public async Task CancellationBeforeDispatchDoesNotCreateArtifacts()
    {
        ...
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => QueryMemoryStorageSelfTest.RunAsync(store.DirectoryPath, null, cancellation.Token));

...
/// </summary>
public sealed class QueryMemoryCapturePerformanceTests
{
    ...
    private static readonly QueryMemoryPolicy RecoveryOnlyPolicy = new(true, false, TimeSpan.FromMinutes(10), true, true);
    ...
    public async Task UnchangedIdleCapturesStopRewritingRecoveryAndCommitFasterThanRealEdits()
    {
        ...
        using (var warmupStore = new SqliteTestStore())
        {
            ...
            await warmupStore.Process(warmupRepository, warmupStore.Capture(1, "SELECT 1;", QueryCaptureKind.DraftIdle), Token, RecoveryOnlyPolicy);
            await warmupStore.Process(warmupRepository, warmupStore.Capture(2, "SELECT 1;", QueryCaptureKind.DraftIdle, seconds: 5), Token, RecoveryOnlyPolicy);
            await warmupStore.Process(warmupRepository, warmupStore.Capture(3, "SELECT 2;", QueryCaptureKind.DraftIdle, seconds: 10), Token, RecoveryOnlyPolicy);
        }

        using var unchangedStore = new SqliteTestStore();
        var unchangedRepository = await unchangedStore.Open(Token);
        await unchangedStore.Process(unchangedRepository, unchangedStore.Capture(1, text, QueryCaptureKind.DraftIdle), Token, RecoveryOnlyPolicy);
        ...
        for (var i = 2; i <= Ticks; i++)
            await unchangedStore.Process(unchangedRepository, unchangedStore.Capture(i, text, QueryCaptureKind.DraftIdle, seconds: i * 5), Token, RecoveryOnlyPolicy);
        unchangedTimer.Stop();
        ...
        await editedStore.Process(editedRepository, editedStore.Capture(1, text, QueryCaptureKind.DraftIdle), Token, RecoveryOnlyPolicy);

        var editedTimer = Stopwatch.StartNew();
        for (var i = 2; i <= Ticks; i++)
            await editedStore.Process(editedRepository, editedStore.Capture(i, text + i, QueryCaptureKind.DraftIdle, seconds: i * 5), Token, RecoveryOnlyPolicy);
        editedTimer.Stop();
